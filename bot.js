/**
SolBot Pro v3.0 — SCALPING MODE
✅ Vente immédiate activée (plus de filtre 'bootstrapped')
✅ Réagit dès le 1er cycle (30s ou moins)
✅ Stop-Loss, Anti-Rug, TP actifs immédiatement
*/
'use strict';
...
// ⏱ Time-Based Stop (Optionnel pour scalping)
...
if (!CONFIG.PRIVATE_KEY) { console.error('❌ PRIVATE_KEY not defined'); process.exit(1); }
...
const safe = String(msg).replace(/PRIVATE_KEY[=:]\S+/gi, 'PRIVATE_KEY=[REDACTED]').replace(/api-key=[^&\s]+/gi, 'api-key=[REDACTED]');
...
body = JSON.stringify({ embeds: [{ title, description: desc, color, fields, footer: { text: `SolBot v${VERSION}` }, timestamp: new Date().toISOString() }] });
...
// ═══════════════════════════════════════════════════════════════════════════
// POSITION MANAGER — SCALPING MODE (NO BOOTSTRAP FILTER)
// ═══════════════════════════════════════════════════════════════════════════
...
// ✅ CHECK SL — NO BOOTSTRAP FILTER
checkSL(mint, price) {
if (!CONFIG.STOP_LOSS_ENABLED||this.isLiquidated(mint)) return null;
const e=this.entries.get(mint); if (!e) return null; // Removed e.bootstrapped check
...
const e=this.entries.get(mint); if (!e||e.bootstrapped) return null; // Time stop can wait
...
if (!r.ok) { lastErr = new Error(`Quote HTTP ${r.status}`); log('warn', 'Quote endpoint failed', { url: url.split('/')[2], status: r.status }); continue; }
...
return q;
} catch (err) { lastErr = err; log('debug', 'Quote endpoint error', { url: url.split('/')[2], err: err.message }); }
}
