...
async function fetchWithRetry(url, options = {}, maxRetries = 2, baseDelay = 1000) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    } catch (err) { if (attempt < maxRetries) { await new Promise(r => setTimeout(r, baseDelay * attempt)); continue; } return null; }
  }
  return null;
}
...
async function getWeightedPrice(mint) {
  ...
  return { priceUsd: sum / totalW, confidence: Math.min(valid.length / sources.length, 1), sources: valid.map(p => p.source), liquidityUsd: valid.find(p => p.liquidityUsd)?.liquidityUsd || 0, change24h: valid.find(p => p.change24h)?.change24h || 0 };
}
...
async function jupiterSell(mint, amountRaw, slippageBps, maxRetries = 3) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (err.message.includes('ENOTFOUND') || err.message.includes('ECONNRESET') || err.message.includes('ETIMEDOUT')) { await new Promise(r => setTimeout(r, Math.pow(2, attempt) * 2000)); continue; }
...
async function executeSell(mint, amountRaw, baseSlippage, reason) {
  ...
  if (failed.count >= 3 && now < failed.nextRetry) { log('info', `[⏸️ VENTE PAUSE] ${mint.slice(0,8)}... : ${failed.count} échecs → Pause`); return null; }
  ...
  try {
    ...
    failed.count++; failed.lastAttempt = now; failed.nextRetry = now + Math.min(120000 * Math.pow(1.5, failed.count - 1), 3600000);
...
async function applySellLogic(mint, balance, decimals, price, pnl, liquidity, change24h) {
  ...
  if (liquidity < 10000) slippage = 500; else if (liquidity < 100000) slippage = 300;
  ...
  for (let i = 0; i < CONFIG.TIERS.length; i++) {
    const t = CONFIG.TIERS[i]; const target = t.targetPnl * mult; const key = `${mint}_tier_${i}`;
    ...
    if (!triggeredTiers[key] && pnl >= target) {
      ...
      if (amt > 0) { log('info', `[🎯 PALIER ${i+1}] ${mint.slice(0,8)}... @ ${pnl.toFixed(2)}% → ${t.sellPercent}%`); await executeSell(mint, amt, slippage, `TAKE_PROFIT_${i+1}`); triggeredTiers[key] = true; }
    }
  }
}
...
async function runCheck() {
  try {
    ...
    for (const mint of allMints) {
      ...
      tokens.push({ symbol: sym, name: meta.name || 'Unknown', address: mint, balance, price, value: balance * price, pnl, liquidity: liq, logo: meta.logo, hasPrice, priceSource: src, priceConfidence: conf, priceSources: pdata?.sources || [], isManual: manualTokens.includes(mint) || dynamicTokens.includes(mint), isInWallet: inWallet, autoSellEnabled: hasPrice && inWallet && balance > 0 });
    }
    lastTokensData = tokens;
    log('debug', `[BOT] ${tokens.length} tokens analysés`);
  } catch (err) {
    log('error', `[BOT] Erreur runCheck : ${err.message}`);
    if (err.message.includes('401') || err.message.includes('api key') || err.message.includes('Unauthorized')) {
      rpcIndex = (rpcIndex + 1) % RPC_URLS.length;
      log('warn', `[RPC] Basculement vers le prochain endpoint`);
    }
  }
}
...
if (process.env.RENDER) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.url === '/api/tokens' && req.method === 'GET') { res.writeHead(200); res.end(JSON.stringify({ success: true, wallet: keypair?.publicKey?.toString(), tokens: lastTokensData, count: lastTokensData.length, timestamp: new Date().toISOString() })); return; }
    if (req.url === '/api/status' && req.method === 'GET') { res.writeHead(200); res.end(JSON.stringify({ status: 'running', wallet: keypair?.publicKey?.toString(), tokensCount: lastTokensData.length, uptime: process.uptime(), autoSell: CONFIG.AUTO_SELL })); return; }
    ...
    if (req.url === '/api/tokens/remove' && req.method === 'DELETE') {
      let body = ''; req.on('data', c => body += c); req.on('end', () => {
        try {
          ...
          if (mi > -1) manualTokens.splice(mi, 1); else if (di > -1) dynamicTokens.splice(di, 1);
