...
/** FIX: fallback v6 appliqué chunk par chunk, pas globalement */
async function batchJupiterPrices(mints) {
  ...
  const lim = pLimit(3);
  await Promise.all(chunks.map(chunk => lim(async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        let count = 0;
        for (const [m, item] of Object.entries(d?.data || {})) {
          const p = parseFloat(item?.price);
          if (p > 0) { results[m] = p; count++; }
        }
        if (count > 0) chunkDone = true;
      }
    } catch {}

    // Essai 2: v6 uniquement si v2 a rien retourné pour CE chunk
    if (!chunkDone) {
      try {
        const r = await fetch(`https://price.jup.ag/v6/price?ids=${chunk.join(',')}`,
          { signal: AbortSignal.timeout(12000) });
        if (r.ok) {
          const d = await r.json();
          for (const [m, item] of Object.entries(d?.data || {})) {
...
async function batchDexScreener(mints) {
  ...
  for (const chunk of chunks) {
    try {
      ...
      // Marque tous les tokens du chunk comme tentés — FIX: évite re-fetch individuel inutile
...
async function prefetchAllPrices(mints) {
  ...
  // 1. Jupiter + DexScreener en parallèle (APIs indépendantes)
  const [jupPrices, dexData] = await Promise.all([
    batchJupiterPrices(toFetch),
    batchDexScreener(toFetch),
  ]);

  // FIX: ne tente individuel que pour tokens JAMAIS vus par le batch DexScreener
  const missing1 = toFetch.filter(m => !jupPrices[m] && !dexData[m] && !batchedSet.has(m));
  if (missing1.length) {
    const lim = pLimit(6);
    await Promise.all(missing1.map(m => lim(async () => {
      try {
        ...
        if (!pairs.length) { batchedSet.add(m); return; }
  ...
  // 2. Pump.fun pour les introuvables avec suffix 'pump'
  const missing2 = toFetch.filter(m => !jupPrices[m] && !dexData[m] && m.endsWith('pump'));
  if (missing2.length) {
    const lim = pLimit(5);
    const pumpRes = {};
    ...
    Object.assign(dexData, pumpRes); // merge pour la fusion ci-dessous
  }

  // 3. Fusionner sources → cache
  for (const mint of toFetch) {
    const dex = dexData[mint];
    const jup = jupPrices[mint];
    let result = null;
    if (dex && dex.price > 0) {
      result = { ...dex, source: dex.source || 'dexscreener' };
      // Jupiter est souvent plus précis pour le prix spot
      if (jup > 0) result.price = jup;
    } else if (jup > 0) {
      result = { price: jup, liquidity: 0, volume24h: 0, change24h: 0,
  ...
  log('debug', 'Prix done', { found, total: toFetch.length, missing: toFetch.length - found });
}
