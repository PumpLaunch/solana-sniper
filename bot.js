/**
 * SolBot Pro v3.1 — Trading Bot Solana Production
 * ✅ Scalping immédiat (pas de filtre bootstrapped sur les sorties)
 * ✅ Take-Profit multi-paliers + Hysteresis
 * ✅ Stop-Loss fixe + Trailing + Break-Even
 * ✅ Anti-Rug + Liquidity Exit protection
 * ✅ DCA buying + Jupiter swap avec fallback endpoints
 * ✅ Webhooks Discord/Telegram
 * ✅ Analytics complets + Persistence JSON
...
// ═══════════════════════════════════════════════════════════════════════════
// LOGGER — Logs sécurisés (masking clés)
// ═══════════════════════════════════════════════════════════════════════════
...
function log(level, msg, data = null) {
  ...
  console.log(`${ICONS[level] || 'ℹ️'} [${new Date().toISOString()}] [${level.toUpperCase()}] ${safe}${sfx}`.trimEnd());
}
...
// ═══════════════════════════════════════════════════════════════════════════
// WALLET & RPC — Chargement + failover
// ═══════════════════════════════════════════════════════════════════════════
function loadWallet() {
  try {
    const raw = CONFIG.PRIVATE_KEY.startsWith('[')
    ...
    log('error', 'Invalid private key', { err: err.message });
    process.exit(1);
  }
}
  ...
  return {
    ...
    async healthCheck() {
      for (let i = 0; i < conns.length; i++) {
        try {
          const s = await conns[i].getSlot();
          if (s > 0) { idx = i; log('debug', 'RPC OK', { slot: s, ep: i }); return true; }
        } catch { log('warn', 'RPC down', { ep: eps[i].slice(0,45) }); }
      }
      log('error', 'All RPC endpoints offline');
      return false;
    },
...
// ═══════════════════════════════════════════════════════════════════════════
// PERSISTENCE — Sauvegarde/Restauration état JSON
// ═══════════════════════════════════════════════════════════════════════════
function loadState() {
  try {
...
// ═══════════════════════════════════════════════════════════════════════════
// PRICE ENGINE — Multi-source cascade (DexScreener → Pump.fun → Birdeye)
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// SCORE ENGINE — Qualité token 0-100
// ═══════════════════════════════════════════════════════════════════════════
class ScoreEngine {
  score(pd) {
    if (!pd) return 0;
...
// ═══════════════════════════════════════════════════════════════════════════
// MOMENTUM TRACKER — Tendance + volatilité
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// POSITION MANAGER — Gestion TP/SL/Trailing/AntiRug/LiqExit
// ═══════════════════════════════════════════════════════════════════════════
class PositionManager {
  ...
  // ✅ STOP-LOSS — SCALPING: réagit immédiatement
  checkSL(mint, price) {
    if (!CONFIG.STOP_LOSS_ENABLED || this.isLiquidated(mint)) return null;
    const e = this.entries.get(mint); if (!e) return null; // ← REMOVED: || e.bootstrapped
    ...
    if (this.breakEven.has(mint)) {
      if (pnl < CONFIG.BREAK_EVEN_BUFFER) return { type: 'break-even', pnl: pnl.toFixed(2), threshold: CONFIG.BREAK_EVEN_BUFFER, sellAmount: rem };
    } else {
      if (pnl > CONFIG.STOP_LOSS_PCT) return null;
      return { type: 'stop-loss', pnl: pnl.toFixed(2), threshold: CONFIG.STOP_LOSS_PCT, sellAmount: rem };
    }
    return null;
  }
...
// ═══════════════════════════════════════════════════════════════════════════
// SWAP ENGINE — Jupiter v6 + Jito bundles
// ═══════════════════════════════════════════════════════════════════════════
...
class SwapEngine {
    ...
    for (const url of QUOTE_ENDPOINTS) {
      try {
        const r = await fetch(url, {
          headers: { 'User-Agent': `SolBot/${VERSION}`, 'Accept': 'application/json', 'Origin': 'https://jup.ag', 'Referer': 'https://jup.ag/' },
          signal: AbortSignal.timeout(15000),
        });
        if (r.status === 401) { log('warn', 'Quote 401', { url: url.split('/')[2] }); continue; }
        if (r.status === 429) { log('warn', 'Quote 429'); await sleep(1000); continue; }
  ...
  async buy(mint, solAmount, slippageBps = CONFIG.DEFAULT_SLIPPAGE) {
    ...
    if (elapsed < CONFIG.BUY_COOLDOWN_MS) throw new Error(`Cooldown: ${((CONFIG.BUY_COOLDOWN_MS - elapsed) / 1000).toFixed(1)}s remaining`);
    const bal = await this.getSolBalance();
    if (bal !== null && bal < solAmount + CONFIG.MIN_SOL_RESERVE) throw new Error(`Solde insuffisant: ${bal.toFixed(4)} SOL (besoin ${(solAmount + CONFIG.MIN_SOL_RESERVE).toFixed(4)})`);
  ...
  async buyDCA(mint, totalSol, chunks, intervalSec, slippageBps = CONFIG.DEFAULT_SLIPPAGE) {
    ...
    for (let i = 0; i < chunks; i++) {
      ...
      } catch (err) { log('warn', `DCA chunk ${i+1} failed`, { err: err.message }); results.push({ chunk: i+1, success: false, error: err.message }); }
    }
  ...
  async sell(mint, amount, reason = 'MANUAL', slippageBps = CONFIG.DEFAULT_SLIPPAGE, useJito = false) {
    if (this.sellFails >= CONFIG.MAX_SELL_RETRIES) {
      const msg = `Circuit-breaker actif (${this.sellFails} échecs)`;
    ...
    try {
      ...
      this.sellFails++;
      log('error', 'Vente échouée', { err: err.message, failures: this.sellFails, reason });
  ...
  async _buildAndSendJito({ inputMint, outputMint, amountRaw, slippageBps }) {
    ...
    try {
      ...
      return { sig, txUrl: `https://solscan.io/tx/${sig}`, quote };
    } catch (err) { log('warn', 'Jito échoué — fallback Jupiter', { err: err.message }); return this._buildAndSendTx({ inputMint, outputMint, amountRaw, slippageBps, priorityMode: 'turbo' }); }
  }
...
// ═══════════════════════════════════════════════════════════════════════════
// ANALYTICS — Sharpe, Sortino, Drawdown, Heatmap
// ═══════════════════════════════════════════════════════════════════════════
class Analytics {
  constructor(state = {}) {
...
class BotLoop {
  ...
  async _sell(mint, sellAmount, reason, priceData, opts = {}) {
    ...
    if (res.success) {
      const symbol = priceData?.symbol || mint.slice(0,8), { pnlSol, pnlPct } = this.recordSell(mint, res.solOut, sellAmount, symbol);
    ...
    try {
      ...
      for (const acc of accounts) {
        ...
        if (price > 0) {
          ...
          // 1. Anti-rug (urgence max, Jito si dispo)
...
// ═══════════════════════════════════════════════════════════════════════════
// API SERVER — 35 routes organisées par domaine
// ═══════════════════════════════════════════════════════════════════════════
function startApi(bot, wallet) {
  ...
  app.get('/api/score/:mint', async (req, res) => {
    ...
    res.json({ mint, score: bot.scorer.score(pd), trend: bot.momentum.getTrend(mint), liqDrop: getLiqDrop(mint),  pd });
  });
...
// ═══════════════════════════════════════════════════════════════════════════
// MAIN — Démarrage + graceful shutdown
// ═══════════════════════════════════════════════════════════════════════════
async function main() {
