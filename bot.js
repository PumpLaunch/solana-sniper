/**
 * SolBot Pro v2.2 — Production Solana Trading Bot
 * ✅ Real Jupiter swaps (NO simulation)
 * ✅ Take-Profit multi-tiers + Hysteresis
 * ✅ Stop-Loss (fixed + trailing)
 * ✅ Anti-Rug protection (instant crash detection)
 * ✅ DCA buying (split orders)
 * ✅ Multi-source prices: DexScreener → Pump.fun → Birdeye
...
const CONFIG = {
  ...
  // 🎯 Take-Profit (enabled by default)
  ...
  // 🛡 Stop-Loss (disabled by default — enable with caution)
  ...
  // 🚨 Anti-Rug (enabled by default — recommended)
...
if (!CONFIG.PRIVATE_KEY) {
  console.error('❌ FATAL: PRIVATE_KEY not set in environment variables');
...
// ═══════════════════════════════════════════════════════════════════════════
// LOGGER — Safe, redacted, production-ready
// ═══════════════════════════════════════════════════════════════════════════
const LEVELS = { info: 'ℹ️', warn: '⚠️', error: '❌', debug: '🔍', success: '✅' };
  ...
  for (let i = 0; i <= retries; i++) {
    try { return await fn(i); }
      ...
      if (i < retries) {
        ...
        log('warn', `${label} retry ${i+1}/${retries} in ${wait}ms`, { error: err.message });
        await sleep(wait);
      }
    }
  }
...
// Concurrency limiter (p-limit alternative)
...
// Mutex for exclusive execution (prevents double-sell)
class Mutex {
...
async function sendWebhook(title, description, color = 0x3b7eff, fields = []) {
  ...
  try {
    ...
    log('warn', 'Webhook failed', { error: err.message });
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// WALLET — Load from PRIVATE_KEY env var
// ═══════════════════════════════════════════════════════════════════════════
function loadWallet() {
  try {
    ...
    log('error', 'Invalid PRIVATE_KEY', { error: err.message });
    process.exit(1);
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// RPC MANAGER — Multi-endpoint with failover
// ═══════════════════════════════════════════════════════════════════════════
function createRpcManager() {
  ...
  const connections = endpoints.map(ep => new Connection(ep, { commitment: 'confirmed' }));
  let idx = 0;
  
  return {
    get connection() { return connections[idx]; },
    
    async healthCheck() {
      for (let i = 0; i < connections.length; i++) {
        try {
          const slot = await connections[i].getSlot();
          if (slot > 0) { idx = i; log('debug', 'RPC OK', { slot, endpoint: i }); return true; }
        } catch { log('warn', 'RPC down', { endpoint: endpoints[i]?.slice(0, 40) }); }
      }
      log('error', 'All RPC endpoints unavailable');
      return false;
    },
    
    failover() {
      idx = (idx + 1) % connections.length;
      log('warn', 'RPC failover', { endpoint: endpoints[idx]?.slice(0, 40) });
    }
  };
}
...
function loadState() {
  ...
  return { entryPrices: {}, trades: [], stopLossHit: [], slPending: [], analytics: {}, costBasis: {} };
}
...
function saveState(state) {
  try {
    ...
    log('warn', 'State save failed', { error: err.message });
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// PRICE ORCHESTRATOR — Multi-source fallback
// ═══════════════════════════════════════════════════════════════════════════
...
// Source 1: DexScreener batch (primary)
async function batchDexScreener(mints) {
  ...
  for (const chunk of chunks) {
    ...
    } catch { /* Chunk failed — individual fallback will handle */ }
    if (chunks.length > 1) await sleep(380);
  }
  return results;
}

// Source 2: Pump.fun (bonding curve tokens)
...
// Source 3: Birdeye (final fallback)
async function fetchBirdeye(mint) {
  try {
    ...
    return { price, liquidity: 0, volume24h: 0, change24h: 0, logo: null, symbol: null, name: null, source: 'birdeye' };
  } catch { return null; }
}
...
async function prefetchAllPrices(mints) {
  ...
  log('debug', 'Fetching prices', { total: toFetch.length });
  ...
  // Fallback 1: DexScreener individual
  const missing1 = toFetch.filter(m => !dexData[m]);
  if (missing1.length) {
    const lim = pLimit(5);
    await Promise.all(missing1.map(m => lim(async () => {
      try {
        const r = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${m}`, { signal: AbortSignal.timeout(8000) });
        if (!r.ok) return;
        const d = await r.json();
        ...
        dexData[m] = {
          ...
          volume24h: best.volume?.h24 || 0, change24h: best.priceChange?.h24 || 0,
          logo: best.info?.imageUrl || null, symbol: best.baseToken?.symbol || null,
          name: best.baseToken?.name || null, source: 'dexscreener-ind'
        };
      } catch {}
    })));
  }
  ...
  for (const mint of toFetch) {
    ...
    if (d && d.price > 0) priceCache.set(mint, {  d, ts: Date.now() });
  }
  
  const found = toFetch.filter(m => (priceCache.get(m)?.data?.price || 0) > 0).length;
  log('debug', 'Prices done', { found, total: toFetch.length, missing: toFetch.length - found });
}

function getTokenPrice(mint) { return priceCache.get(mint)?.data || null; }

// ═══════════════════════════════════════════════════════════════════════════
// POSITION MANAGER — TP, SL, Trailing, Anti-Rug logic
// ═══════════════════════════════════════════════════════════════════════════
class PositionManager {
  constructor(tiers, hysteresis, savedState = {}) {
    ...
    log('info', 'Positions restored', { count: this.entryPrices.size });
  }
  ...
  trackEntry(mint, currentPrice, currentBalance) {
    ...
    log('info', 'Position opened', { mint: mint.slice(0,8), entry: currentPrice.toPrecision(6) });
    return true;
  }
  ...
  updatePrevPrice(mint, price) { if (price > 0) this.prevPrices.set(mint, price); }
  ...
  markTierExecuted(mint, tierIndex, amountSold) {
    ...
    log('success', `TP tier ${tierIndex+1} executed`, { mint: mint.slice(0,8), sold: amountSold.toFixed(4) });
  }
  ...
  markStopLossPending(mint) {
    ...
    log('warn', 'SL marked pending (sale failed)', { mint: mint.slice(0,8) });
  }
  
  clearStopLossPending(mint) { this.slPending.delete(mint); }
  ...
  toSerializable() {
    ...
    for (const [mint, data] of this.entryPrices) {
      out[mint] = {
        price: data.price, ts: data.ts, originalBalance: data.originalBalance,
  ...
  getStats() {
    ...
    for (const [mint, data] of this.entryPrices) {
      ...
      entries.push({
        mint: mint.slice(0,8) + '...', mintFull: mint,
        entryPrice: data.price, originalBalance: data.originalBalance,
        sold: this.soldAmounts.get(mint) || 0, remaining: this.getRemainingBalance(mint),
        triggeredTiers: Array.from(triggered).map(i => this.tiers[i]?.pnl),
        stopLossHit: this.stopLossHit.has(mint), slPending: this.slPending.has(mint),
        peakPnl: this.peakPnl.get(mint) || 0
      });
    }
    ...
    return {
      ...
      tracked: entries.length, entries
    };
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// SWAP ENGINE — Jupiter v6 (REAL trades)
// ═══════════════════════════════════════════════════════════════════════════
class SwapEngine {
  ...
  async getQuote({ inputMint, outputMint, amountRaw, slippageBps }) {
    ...
    for (const url of ENDPOINTS) {
      try {
        ...
        if (!q.outAmount) { lastErr = new Error('No quote output'); continue; }
        return q;
      } catch (err) { lastErr = err; }
    }
    throw lastErr || new Error('All quote endpoints failed');
  }
  
  async _executeSwap({ inputMint, outputMint, amountRaw, slippageBps }) {
    return withRetry(async () => {
      // Refetch quote each attempt (quotes expire ~60s)
      ...
      if (!swapData?.swapTransaction) throw swapErr || new Error('All swap endpoints failed');
      
      const tx = VersionedTransaction.deserialize(Buffer.from(swapData.swapTransaction, 'base64'));
      const txBlockhash = tx.message.recentBlockhash; // Extract from Jupiter tx
      ...
      const conf = await this.rpc.connection.confirmTransaction({
        signature: txId, blockhash: txBlockhash, lastValidBlockHeight: lbhMeta.lastValidBlockHeight
      }, 'confirmed');
  ...
  async buy(mint, solAmount, slippageBps = CONFIG.DEFAULT_SLIPPAGE) {
    ...
    if (elapsed < CONFIG.BUY_COOLDOWN_MS) {
      ...
      throw new Error(`Buy cooldown — retry in ${wait}s`);
    }
    ...
    if (solBal !== null) {
      ...
      if (solBal < needed) {
        throw new Error(`Insufficient SOL: ${solBal.toFixed(4)} (need ${needed.toFixed(4)})`);
      }
    }
    
    log('info', 'Buying', { mint: mint.slice(0,8), solAmount, slippageBps });
    ...
    log('success', 'Buy confirmed', { mint: mint.slice(0,8), out: outAmount.toFixed(4), txId });
  ...
  async buyDCA(mint, totalSol, chunks, intervalSec, slippageBps = CONFIG.DEFAULT_SLIPPAGE) {
    ...
    for (let i = 0; i < chunks; i++) {
      try {
        ...
        log('warn', `DCA chunk ${i+1} failed`, { error: err.message });
  ...
  async sell(mint, amount, reason = 'MANUAL', slippageBps = CONFIG.DEFAULT_SLIPPAGE) {
    ...
    if (this.sellFailures >= CONFIG.MAX_SELL_RETRIES) {
      const msg = `Circuit breaker active (${this.sellFailures} failures) — restart bot`;
      log('error', msg);
    ...
    try {
      log('info', 'Selling', { mint: mint.slice(0,8), amount: amount.toFixed(4), reason });
      const outDec = await getTokenDecimals(mint, this.rpc.connection);
      ...
      log('success', 'Sell confirmed', { mint: mint.slice(0,8), solOut: solOut.toFixed(6), txId, reason });
      return { success: true, txId, txUrl, solOut, amountSold: amount };
      
    } catch (err) {
      this.sellFailures++;
      log('error', 'Sell failed', { error: err.message, failures: this.sellFailures, reason });
  ...
  async getSolBalance() {
    try { return await this.rpc.connection.getBalance(this.wallet.publicKey) / 1e9; }
    catch { return null; }
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// BOT LOOP — Main trading cycle
// ═══════════════════════════════════════════════════════════════════════════
class BotLoop {
  constructor(wallet, rpc, savedState) {
    ...
    this.analytics = {
      ...
      totalBoughtSol: saved.totalBoughtSol || 0,
      totalSoldSol: saved.totalSoldSol || 0,
      winCount: saved.winCount || 0,
      lossCount: saved.lossCount || 0,
      totalTrades: saved.totalTrades || 0,
      bestTradePct: saved.bestTradePct || null,
      worstTradePct: saved.worstTradePct || null,
      bestTradeSymbol: saved.bestTradeSymbol || null,
      worstTradeSymbol: saved.worstTradeSymbol || null,
      avgHoldMs: saved.avgHoldMs || 0,
  ...
  _recordSell(mint, solOut, amountSold, symbol) {
    ...
    if (cb && cb.solSpent > 0 && cb.tokBought > 0) {
      ...
      if (this.analytics.dailyPnl.length > 90) this.analytics.dailyPnl.shift();
  ...
  async tick() {
    try {
      ...
      for (const acc of allAccounts) {
        ...
        if (price > 0) {
          // ── Anti-Rug (highest priority) ─────────────────────────────
          const rugAlert = this.positions.checkAntiRug(mint, price);
          if (rugAlert) {
            log('error', '🚨 ANTI-RUG TRIGGERED', { mint: mint.slice(0,8), drop: rugAlert.drop + '%' });
            await sendWebhook('🚨 Anti-Rug Alert', `**${rugAlert.drop}%** drop detected`, 0xff4757, [
              { name: 'Token', value: priceData?.symbol || mint.slice(0,8), inline: true },
              { name: 'Drop', value: rugAlert.drop + '%', inline: true }
            ]);
            
            this.positions.markStopLossPending(mint);
            const res = await this.swap.sell(mint, rugAlert.sellAmount, 'ANTI_RUG', 1000); // High slippage
            ...
            if (res.success) {
              ...
              await sendWebhook('✅ Anti-Rug sold', `${rugAlert.sellAmount.toFixed(4)} tokens sold`, 0x05d488, [
                { name: 'SOL received', value: res.solOut?.toFixed(6), inline: true }
              ]);
            } else {
          ...
          if (CONFIG.TAKE_PROFIT_ENABLED && pnl !== null) {
            ...
            for (const tier of tiers) {
              log('warn', `TP tier ${tier.tierIndex+1}`, {
              ...
              if (res.success) {
                ...
                this._recordTrade({ type: 'sell', mint, symbol: priceData?.symbol || mint.slice(0,8),
                  amount: tier.sellAmount, solOut: res.solOut, reason: `TP Tier ${tier.tierIndex+1}`,
                  txId: res.txId, txUrl: res.txUrl, pnlSol, pnlPct });
                await sendWebhook(`🎯 Take-Profit T${tier.tierIndex+1}`,
                  `+${tier.currentPnl}% reached on **${priceData?.symbol || mint.slice(0,8)}**`, 0x05d488, [
                  { name: 'Sold', value: tier.sellAmount.toFixed(4), inline: true },
                  { name: 'SOL received', value: res.solOut?.toFixed(6), inline: true }
                ]);
              }
            }
          ...
          if (sl) {
            ...
            if (res.success) {
              this.positions.markStopLossExecuted(mint);
              const { pnlSol, pnlPct } = this._recordSell(mint, res.solOut, sl.sellAmount, priceData?.symbol);
              this._recordTrade({ type: 'sell', mint, symbol: priceData?.symbol || mint.slice(0,8),
                amount: sl.sellAmount, solOut: res.solOut, reason: 'Stop-Loss',
                txId: res.txId, txUrl: res.txUrl, pnlSol, pnlPct });
              await sendWebhook('🔴 Stop-Loss triggered',
                `**${priceData?.symbol || mint.slice(0,8)}** sold at ${sl.pnl}%`, 0xff4757, [
                { name: 'SOL recovered', value: res.solOut?.toFixed(6), inline: true }
              ]);
            } else {
              this.positions.clearStopLossPending(mint);
            }
          ...
          if (ts) {
            ...
            if (res.success) {
              this.positions.markStopLossExecuted(mint);
              const { pnlSol, pnlPct } = this._recordSell(mint, res.solOut, ts.sellAmount, priceData?.symbol);
              this._recordTrade({ type: 'sell', mint, symbol: priceData?.symbol || mint.slice(0,8),
                amount: ts.sellAmount, solOut: res.solOut, reason: 'Trailing Stop',
                txId: res.txId, txUrl: res.txUrl, pnlSol, pnlPct });
              await sendWebhook('📉 Trailing Stop',
                `**${priceData?.symbol || mint.slice(0,8)}** — peak: +${ts.peak}%, current: ${ts.pnl}%`, 0xffb020);
            } else {
              this.positions.clearStopLossPending(mint);
            }
          }
        ...
        tokens.push({
          ...
          triggeredTiers: Array.from(this.positions.triggeredTiers.get(mint) || []).map(i => CONFIG.TAKE_PROFIT_TIERS[i]?.pnl),
...
function startApi(bot, wallet) {
  ...
  // CORS — wildcard for API (no cookies/auth)
  ...
  if (hasIndex) {
    ...
    log('info', 'Dashboard served from /public');
  } else {
    app.get('/', (req, res) => {
      res.json({
        ...
        dashboard: CONFIG.DASHBOARD_URL || 'Not configured',
  ...
  app.post('/api/config', (req, res) => {
    const { takeProfitEnabled, takeProfitTiers, hysteresis, stopLossEnabled, stopLossPct,
            trailingEnabled, trailingPct, antiRugEnabled, antiRugPct, defaultSlippage, minSolReserve, intervalSec } = req.body;
    ...
    const sl = num(stopLossPct, -100, 0); if (sl !== null) CONFIG.STOP_LOSS_PCT = sl;
    const tr = num(trailingPct, 1, 100); if (tr !== null) CONFIG.TRAILING_STOP_PCT = tr;
    const ar = num(antiRugPct, 1, 100); if (ar !== null) CONFIG.ANTI_RUG_PCT = ar;
    const ds = num(defaultSlippage, 10, 5000); if (ds !== null) CONFIG.DEFAULT_SLIPPAGE = ds;
    const mr = num(minSolReserve, 0, 10); if (mr !== null) CONFIG.MIN_SOL_RESERVE = mr;
    const hys = num(hysteresis, 0, 50); if (hys !== null) CONFIG.TAKE_PROFIT_HYSTERESIS = hys;
    const ivl = num(intervalSec, 10, 3600); if (ivl !== null) CONFIG.INTERVAL_SEC = ivl;
    
    log('info', 'Config updated', {
      tp: CONFIG.TAKE_PROFIT_ENABLED, sl: CONFIG.STOP_LOSS_ENABLED,
      trail: CONFIG.TRAILING_STOP_ENABLED, antiRug: CONFIG.ANTI_RUG_ENABLED
    });
    
    res.json({ success: true, config: {
      takeProfitEnabled: CONFIG.TAKE_PROFIT_ENABLED, takeProfitTiers: CONFIG.TAKE_PROFIT_TIERS,
      hysteresis: CONFIG.TAKE_PROFIT_HYSTERESIS, stopLossEnabled: CONFIG.STOP_LOSS_ENABLED,
      stopLossPct: CONFIG.STOP_LOSS_PCT, trailingEnabled: CONFIG.TRAILING_STOP_ENABLED,
      trailingPct: CONFIG.TRAILING_STOP_PCT, antiRugEnabled: CONFIG.ANTI_RUG_ENABLED,
      antiRugPct: CONFIG.ANTI_RUG_PCT, defaultSlippage: CONFIG.DEFAULT_SLIPPAGE,
      minSolReserve: CONFIG.MIN_SOL_RESERVE, intervalSec: CONFIG.INTERVAL_SEC
    }});
  });
  ...
  app.post('/api/buy', async (req, res) => {
    ...
    if (isNaN(sol) || sol <= 0 || sol > 50) return res.status(400).json({ error: 'solAmount invalid (0-50 SOL)' });
  ...
  app.post('/api/buy/dca', async (req, res) => {
    ...
    if (isNaN(sol) || sol <= 0) return res.status(400).json({ error: 'totalSol invalid' });
  ...
  app.post('/api/sell', async (req, res) => {
    ...
    if (!tok) return res.status(404).json({ error: 'Token not found in portfolio' });
    
    let sellAmount = amount ? parseFloat(amount) : 0;
    if (percent !== undefined) sellAmount = tok.balance * (parseFloat(percent) / 100);
    if (!sellAmount || sellAmount <= 0) return res.status(400).json({ error: 'Invalid amount' });
    sellAmount = Math.min(sellAmount, tok.balance);
    
    const result = await bot.swap.sell(tok.mintFull, sellAmount, reason, parseInt(slippageBps) || CONFIG.DEFAULT_SLIPPAGE);
    ...
    if (result.success) {
      ...
      bot._recordTrade({ type: 'sell', mint: tok.mintFull, symbol: tok.symbol || tok.mintFull.slice(0,8),
        amount: sellAmount, solOut: result.solOut, reason, txId: result.txId, txUrl: result.txUrl, pnlSol, pnlPct });
      bot._persist();
      setTimeout(() => bot.tick().catch(() => {}), 4000);
    }
  ...
  app.get('/api/analytics', (req, res) => {
    ...
    const roi = a.totalBoughtSol > 0 ? +((a.realizedPnlSol / a.totalBoughtSol) * 100).toFixed(2) : null;
    ...
    res.json({
      realizedPnlSol: +a.realizedPnlSol.toFixed(4), totalBoughtSol: +a.totalBoughtSol.toFixed(4),
      totalSoldSol: +a.totalSoldSol.toFixed(4), winCount: a.winCount, lossCount: a.lossCount,
      totalTrades: a.totalTrades, winRate, roi, avgWin, avgLoss, avgHold,
      bestTradePct: a.bestTradePct, bestTradeSymbol: a.bestTradeSymbol,
      worstTradePct: a.worstTradePct, worstTradeSymbol: a.worstTradeSymbol,
      dailyPnl: a.dailyPnl.slice(-30), pnlHistory: a.pnlHistory.slice(-200)
    });
  });
  ...
  app.listen(CONFIG.PORT, '0.0.0.0', () =>
    log('info', 'API started', { port: CONFIG.PORT, version: VERSION })
  );
...
async function main() {
  ...
  log('info', 'First cycle...');
  await bot.tick();
  
  setInterval(() => bot.tick().catch(err => log('error', 'Loop error', { error: err.message })), CONFIG.INTERVAL_SEC * 1000);
  ...
  log('success', 'Bot operational', {
    ...
    tp: CONFIG.TAKE_PROFIT_ENABLED ? `${CONFIG.TAKE_PROFIT_TIERS.length} tiers` : 'off',
  ...
  // Graceful shutdown
  const cleanup = () => { bot._persist(); log('info', 'Graceful shutdown'); process.exit(0); };
