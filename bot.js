 ...
 * ✅ Fix: batchJupiterPrices fallback
 * ✅ Fix: Blockhash extrait du tx Jupiter
 * ✅ Fix: stopLossHit persisté
 * ✅ Fix: Retry _executeSwap refetch quote
 * ✅ Fix: Stop-loss marque "pending" si échec
 * ✅ Perf: DexScreener individuel optimisé
 * ✅ Feat: Trailing stop-loss
 * ✅ Feat: Achat DCA
 * ✅ Feat: Webhook notifications
 * ✅ Feat: Détection anti-rug
 * ✅ Feat: GET /api/config
 * ✅ Feat: Statistiques PnL globales
 */
'use strict';
...
function startApi(bot, wallet) {
  ...
  app.post('/api/config', (req, res) => {
    const b = req.body;
    if (b.takeProfitEnabled !== undefined) CONFIG.TAKE_PROFIT_ENABLED   = !!b.takeProfitEnabled;
    if (b.stopLossEnabled   !== undefined) CONFIG.STOP_LOSS_ENABLED     = !!b.stopLossEnabled;
    if (b.trailingEnabled   !== undefined) CONFIG.TRAILING_STOP_ENABLED = !!b.trailingEnabled;
    if (b.antiRugEnabled    !== undefined) CONFIG.ANTI_RUG_ENABLED      = !!b.antiRugEnabled;
    
    if (Array.isArray(b.takeProfitTiers) && b.takeProfitTiers.length > 0) {
    ...
    const sl  = validateNum(b.stopLossPct,    -100, 0);    if (sl  !== null) CONFIG.STOP_LOSS_PCT           = sl;
    const tr  = validateNum(b.trailingPct,    1, 100);     if (tr  !== null) CONFIG.TRAILING_STOP_PCT       = tr;
    const ar  = validateNum(b.antiRugPct,     1, 100);     if (ar  !== null) CONFIG.ANTI_RUG_PCT            = ar;
    const ds  = validateNum(b.defaultSlippage,10, 5000);   if (ds  !== null) CONFIG.DEFAULT_SLIPPAGE        = ds;
    const mr  = validateNum(b.minSolReserve,  0, 10);      if (mr  !== null) CONFIG.MIN_SOL_RESERVE         = mr;
    const hys = validateNum(b.hysteresis,     0, 50);      if (hys !== null) CONFIG.TAKE_PROFIT_HYSTERESIS  = hys;
    const ivl = validateNum(b.intervalSec,    10, 3600);   if (ivl !== null) CONFIG.INTERVAL_SEC            = ivl;
  ...
  app.get('/api/analytics', (req, res) => {
    ...
    res.json({
      ...
      winRate, roi, avgWin, avgLoss, avgHold,
