/**
 * 🤖 SolBot-Basic v1.3.5 — SIMPLE MODE: Direct DexScreener fetch per token
 */
'use strict';
...
// ═══════════════════════════════════════════════════════════════════════════
// FETCH PRIX — DEXSCREENER UNIQUEMENT (Simple & Direct)
// ═══════════════════════════════════════════════════════════════════════════
...
async function fetchPriceFromDexScreener(mint) {
  ...
  try {
    ...
    if (!res.ok) {
      log('debug', 'DexScreener HTTP error', { mint: cleanMint.slice(0,10)+'...', status: res.status });
      return null;
    }
    ...
    return {
      ...
      source: 'dexscreener-simple'
    };
    
  } catch (err) {
    log('debug', 'DexScreener fetch error', { mint: String(mint||'').slice(0,10)+'...', error: err.message });
    return null;
  }
}
...
class SellExecutor {
  ...
  async executeJupiterSell(mint, amount, slippageBps = 500) {
    try {
      ...
      const cleanMint = String(mint||'').trim();
      const quoteRes = await fetch(`https://quote-api.jup.ag/v6/quote?inputMint=${cleanMint}&outputMint=${SOL_MINT}&amount=${amountRaw}&slippageBps=${slippageBps}&maxAccounts=20`, {
        headers: { 'User-Agent': `SolBot-Basic/${VERSION}` }, signal: AbortSignal.timeout(30000)
      });
...
class BotLoop {
  ...
  async tick() {
    try {
      ...
      log('info', '🔁 Début cycle', { totalAccounts: allAccounts.length });
      
      const tokens = [];
      let totalValue = 0;
      let withPrice = 0;
      
      // Process ALL tokens (no slice limit)
      for (const acc of allAccounts) {
        ...
        // ✅ Fetch price DIRECTLY from DexScreener (no cache, no batch)
        ...
        // Petit délai pour éviter rate limiting DexScreener
      ...
      log('error', '❌ Erreur cycle', { error: err.message, stack: err.stack?.slice(0,300) });
    }
  }
...
async function main() {
  log('info', `🤖 SolBot-Basic ${VERSION} — SIMPLE MODE`, { env: CONFIG.NODE_ENV });
  ...
  // Cycle toutes les 45s (plus lent pour éviter rate limiting)
  setInterval(() => bot.tick().catch(err => log('error', 'Erreur loop', { error: err.message })), 45000);
  
  startApi(bot, wallet);
  log('info', '✅ Bot actif', { address: wallet.publicKey.toString().slice(0,8)+'...', interval: '45s' });
